...
`ssh -i ~/.ssh/id_bastion -t usrbastion@srv-bastion.example.com 'ssh -i ~/.ssh/id_lan -p 28 appusr@192.168.1.3'`

Connecting to destination 192.168.1.3 via srv-bastion in ssh config 
...
Host 192.168.1.3
  ...
  User usrbastion
  RequestTTY force
  RemoteCommand ssh -i ~/.ssh/id_lan -p 28 appusr@192.168.1.3
#+END_SRC
