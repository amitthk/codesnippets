...
docker push asia-southeast1-docker.pkg.dev/stridecal-web-dev/stridecal-docker/eureka-server:latest
docker images | grep asia-southeast1-docker.pkg.dev | awk '{print "docker push "$1":"$2}' | bash


gcloud container clusters list
gcloud projects list
gcloud config set project stridecal-web-dev-409409
gcloud container clusters list
...
#+BEGIN_SRC 
kubectl create deployment eureka-server --image=asia-southeast1-docker.pkg.dev/stridecal-web-dev-409409/stridecal-docker/eureka-server:latest --dry-run=client -o yaml
#+END_SRC
...
#+BEGIN_SRC 
gcloud artifacts repositories add-iam-policy-binding stridecal-docker \
...
#+BEGIN_SRC 
docker buildx build --platform linux/amd64  -t us-docker.pkg.dev/stridecal-web-dev/stridecal-docker/config-server:latest .
#+END_SRC
...
docker buildx build --platform linux/amd64 -t 197991497670.dkr.ecr.ap-southeast-1.amazonaws.com/stridecal-smtp:latest .
